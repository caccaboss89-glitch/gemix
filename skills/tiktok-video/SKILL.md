...
- Anything else (a changed embed shape, a yt-dlp error): it already tried both paths. Run it once more only if the reason names a timeout or a transient network error. If it fails again, tell the user TikTok is not serving that one right now.
