# Grok Build System Prompt (leaked, source: asgeirtj/system_prompts_leaks)
...
## Key Structural Observations for GemiX

1. **user_query tag**: The last user message is wrapped in `<user_query>` — making the model's goal explicit. GemiX currently just appends the Runtime context as the last user message, leaving ambiguity about what to answer.

2. **Free-text prompt with XML only for dynamic content**: Grok Build uses plain English prose for instructions, with XML tags ONLY around dynamic/runtime data (user_query, system-reminder for AGENTS.md, skills manifest, MCP servers). Not for the instructions themselves.

3. **Action safety as a named section**: Not buried in Directives — it's a top-level `<action_safety>` block that explains the philosophy first, then gives examples.

4. **Tool descriptions are self-contained**: Each tool's description tells you everything you need to know about using it — no cross-references to system prompt sections needed.

5. **No PreSendCheck/Directives/numbered rules**: Instructions are organized by topic in named sections, not numbered rules with a check phase.

6. **No voice/attachments/title schema in prompt**: Response schema is defined by the API request body, not described in the prompt text.

7. **Edit/validation philosophy**: "old_string must match exactly one place in the file" — precise, minimal, no room for ambiguity.
