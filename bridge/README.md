...
tool variant on `/responses`).

Hermes itself, however, ships internal toolsets `image_gen` and `video_gen`
that DO work — but only through the Hermes CLI / TUI, not the proxy. The
one-shot mode (`hermes -z "<prompt>"`) calls those tools and prints the
resulting media URL on stdout, which is exactly the contract we need.
