...
const TOOL_INSTRUCTIONS = {
  ...
  send_voice_message: `Genera un messaggio vocale (OGG/Opus). Il parametro "text" è il contenuto da convertire (max 1000 caratteri). Usa i tag vocali per effetti. Non inviare altra risposta testuale quando chiami questo tool. ${VOICE_EFFECTS_DOC}`,
  schedule_tasks: `Programma attività future. Ogni task include taskType (static/dynamic), content, scheduledAt (ISO 8601 Europe/Rome) e destinazioni (WhatsApp privato, gruppo, email). In modalità dynamic, content è un prompt per Grok.`,
  read_my_tasks: `Mostra i task programmati dell'utente corrente (e opzionalmente del gruppo corrente).`,
