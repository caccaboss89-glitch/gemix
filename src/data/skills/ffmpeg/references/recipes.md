...
Extended command cookbook. Read this file when the core recipes in `SKILL.md`
don't cover the task, or when dealing with complex `filter_complex`, multi-input,
or advanced stream mapping.

Sandbox reminders (from `SKILL.md`): every `bash` call is a fresh shell, so run a
multi-line recipe as a **single** command and define `$INPUT`/`$OUTPUT` (or use
literal `/workspace/...` paths) inside it. Quote every path. When re-encoding, add
`-threads 4` and, for long/heavy jobs, raise `timeout_ms` (≤120000). ffmpeg reads
local files only — never a web URL.

---

## Handling Missing Audio Streams

Some inputs have no audio. Commands that map audio streams (`-map 0:a`,
`-filter_complex "[0:a]..."`) will fail on audio-less inputs.

**Rules:**

- When mapping audio, use optional syntax: `-map 0:a:0?`
- When the task involves audio mixing, speed change, or concat with audio, probe
  first to confirm audio exists.
...
If this fails, the input codecs are not compatible with the output container --
re-encode.
...
ffmpeg -n -i "$INPUT" \
  ...
  -c:v libx264 -crf 23 -preset medium -threads 4 -c:a aac -b:a 128k \
  "$OUTPUT"
```
...
If video stream copy fails, re-encode with `-c:v libx264 -crf 23 -preset medium -threads 4`.
...
Use a temp palette in a temp directory to avoid clobbering user files (run the
whole block in one bash call so the temp dir and trap apply):
...
Re-encodes everything. All inputs must have audio; if any lacks audio, add a silent
track first (see "Handling Missing Audio Streams" above).
...
ffmpeg -n \
  ...
  -map "[v]" \
  -c:v libx264 -crf 23 -preset medium -threads 4 -pix_fmt yuv420p \
  "$OUTPUT"
```
...
For video: lower setpts = faster. For audio: higher atempo = faster. `atempo`
supports 0.5-100.0; for extreme changes chain multiple filters (e.g.
`atempo=2.0,atempo=2.0` for 4x).
...
If font errors occur, specify a font file with `fontfile=/path/to/font.ttf`. For
user-supplied text, use `drawtext=textfile=input.txt` instead of inline `text=` to
avoid escaping issues with quotes, colons, and special characters. Write the text in
the user's language and avoid emoji (default fonts render them as boxes).
...
fetched with `web_search` and saved via `curl -L -o /workspace/... URL` (bash).
...
ffmpeg -n -i "$INPUT" \
  -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" \
  -c:v libx264 -crf 23 -preset medium -threads 4 -c:a copy \
  "$OUTPUT"
```
...
90 CW: `-vf "transpose=1"`. 90 CCW: `-vf "transpose=2"`. 180:
...
Use `fontsdir=.` if custom fonts are in the current directory. ASS subtitles
(`subs.ass`) support richer styling natively.
...
```bash
ffmpeg -n -ss "00:00:07" -i "$INPUT" -frames:v 1 -q:v 2 /workspace/thumbnail.jpg
```

PNG (lossless):

```bash
ffmpeg -n -ss "00:00:07" -i "$INPUT" -frames:v 1 /workspace/thumbnail.png
```
...
ffmpeg -n -i "$INPUT" \
  -vf "fps=1/10,scale=320:-1,tile=3x3" \
  -frames:v 1 \
  /workspace/storyboard.jpg
```

Adjust `fps=1/N` for density (1/5 = every 5s, 1/30 = every 30s). Change `tile=CxR`
for grid size.
...
ffmpeg -n -framerate 1/3 \
  -pattern_type glob -i "/workspace/slides/*.jpg" \
  -c:v libx264 -crf 23 -pix_fmt yuv420p -threads 4 \
  -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" \
  /workspace/slideshow.mp4
```

`-framerate 1/3` means each image is shown for 3 seconds. Input images should have
the same dimensions. If they differ, use `scale` + `pad` to fit a fixed canvas (e.g.
`scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:black`).
The `scale` filter alone only ensures even dimensions, it does not normalize
different sizes.
...
ffmpeg -n -framerate 1/3 \
  -pattern_type glob -i "/workspace/slides/*.jpg" \
  -i "/workspace/background_music.mp3" \
  -c:v libx264 -crf 23 -pix_fmt yuv420p -threads 4 \
  -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" \
  -c:a aac -shortest \
  /workspace/slideshow.mp4
...
ffmpeg -n -i "$INPUT" \
  ...
  -c:v libx264 -crf 23 -preset medium -threads 4 -c:a aac -b:a 128k -pix_fmt yuv420p \
  "$OUTPUT"
```
...
Letterbox/pillarbox (fit into canvas with padding) -- see "Fit Into a Canvas With
Padding" above.
