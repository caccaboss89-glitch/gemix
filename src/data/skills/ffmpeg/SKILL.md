...
- **ffmpeg reads local files only.** Web video/audio: `yt-dlp` into `/workspace/`, then process here.
...
2. PNGs you render yourself in the sandbox (e.g. a matplotlib chart, then overlay
   or sequence it).
3. Images from `web_search`: `curl -L -o /workspace/... URL` via bash.
...
`web_search` for images (`curl -L -o`) or render it yourself, then composite it.
