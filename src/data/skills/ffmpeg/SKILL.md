---
name: ffmpeg
description: Process media files in the workspace with ffmpeg/ffprobe — inspect, convert, trim, resize/crop/pad, compress, change codec/frame-rate/speed, extract or replace audio, mute, extract frames/thumbnails, make GIFs, add subtitles/overlays/logos/text, build slideshows, and combine/merge/join/stitch/concatenate clips end-to-end. Use whenever the deliverable is a video, audio, GIF, or frame extracted in /workspace/. Do NOT use to merely watch/listen to a media file (read_file already shows that) or to download a video from the web (that is yt-dlp's job).
---
...
2. PNGs you render yourself in the sandbox (e.g. a matplotlib chart, then overlay
   or sequence it).
3. Images fetched from the web with `web_x_search` (`search_images=true`): they land
   in `/workspace/` and the tool returns their filenames.
...
would make the video clearer or more polished and the task didn't supply one, fetch
it with `web_x_search search_images=true` or render it yourself, then composite it.
