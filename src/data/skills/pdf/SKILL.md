---
name: pdf
description: Create or edit PDFs in /workspace/ with reportlab, pypdf, or poppler. Not for LaTeX PDFs (compile with pdflatex in /workspace/).
---
...
- **write_file / edit_file JSON**: escape backslashes in tool args, but the `.tex`
  on disk must use normal LaTeX (one `\` per command). Before `pdflatex`, verify
  with `head -c 200 file.tex | cat -A`; if you see `\\documentclass`, fix
  escapes — do not burn rounds on blind rewrites.
- **Setup**: one bash checks `pdflatex` + `kpsewhich` for the packages you need —
  not many separate `kpsewhich` rounds.
- **Compile**: 2–3 `pdflatex` passes for TOC/cross-refs; set bash `timeout_ms`
  120000+ on large jobs.
